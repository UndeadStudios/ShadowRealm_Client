...
//    }
    ...
    public final void renderAtPoint(int orientation, int pitchSine, int pitchCos, int yawSin, int yawCos, int offsetX, int offsetY, int offsetZ, long uid) {
        ...
        int objX = (xRot - diagonal2DAboveOrigin) * WorldController.focalLength;
        if (objX / pos >= DrawingArea.viewportCenterX)
            return;

        int objWidth = (xRot + diagonal2DAboveOrigin) * WorldController.focalLength;
        if (objWidth / pos <= -DrawingArea.viewportCenterX)
            return;
        ...
        int objHeight = (yRot + var20) * WorldController.focalLength;
        if (objHeight / pos <= -DrawingArea.viewportCenterY)
            return;

        int offset = dimensionCosY + (super.modelHeight * pitchCos >> 16);
        int objY = (yRot - offset) * WorldController.focalLength;
        if (objY / pos >= DrawingArea.viewportCenterY)
            return;
    ...
    public void decode662(byte abyte0[], int modelID) {
        ...
        ModelDef_1.triangleCount = verticesCount;
